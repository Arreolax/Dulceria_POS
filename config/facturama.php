...
function facturama_request(string $method, string $path, array $fields = [], ?array $jsonBody = null): array {
    ...
    // Generación del token en Base64
    $credenciales = FacturamaCfg::user() . ':' . FacturamaCfg::pass();
    $token = base64_encode($credenciales);
...
function facturama_create_cfdi(array $fields): array {
    // Enviamos los campos en el cuarto parámetro ($jsonBody) para forzar application/json
    return facturama_request('POST', '/3/cfdis', [], $fields);
}
...
function facturama_download_issued(string $type, string $facturamaId): array {
    ...
    // Generación del token en Base64 para la descarga
    $credenciales = FacturamaCfg::user() . ':' . FacturamaCfg::pass();
    $token = base64_encode($credenciales);
